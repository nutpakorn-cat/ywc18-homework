<template>
  <div>
    <EachResult 
      v-for="(result, index) in dummyMerchantAPI" 
      :key="index" 
      :payload="result"
    />
  </div>
...
<script>
  export default {
    name: 'SearchedResultList',
    props: ['categoryName', 'subCategoryName'],
    created() {
      this.dummyMerchantAPI = this.dummyMerchantAPI.filter(
        (each) => (each.subcategoryName === this.subCategoryName) || (this.subCategoryName === 'ทั้งหมด')
      );
    },
    data() {
      return {
